...
- Project-local Impeccable is pinned under `.agents/skills/impeccable/**` for Codex and `.agent/skills/impeccable/**` for Antigravity. Automatic Impeccable hooks remain off because the authenticated Nyan hook is authoritative. The runner must verify the tracked skill/version in the exact UI worktree, and the UI prompt invokes `/impeccable polish`; the skill never widens the file grant.
