...
- Nyan is the product owner, trial user, and the only authority for live deployment or real transactions. Do not ask Nyan to relay output between agents or manually dispatch ordinary ready work; route findings and out-of-scope needs through the coordinator.
...
- Keep changes inside the issue scope, link real dependencies, and attach exact-SHA test evidence. Every author, including Codex, receives an independent review of the current exact HEAD. A worker result or open PR is not Done; Done requires the gated commit to be merged. Nyan performs the final merge.
...
- Codex/coordinator owns frontend architecture: app wiring, theme mechanism, domain and API contracts, data adapters, generated types/fixtures, routing, shared components, dependencies, manifests, lockfiles, build/test configuration, Docker/nginx, CI, runner policy, and this instruction file.
...
- Product and design truth live in the single root `PRODUCT.md` and `DESIGN.md` pair. The admin is an operate-mode, basic, light-first tool: default to light regardless of OS preference, persist only an explicit user choice, prevent wrong-theme paint, and fall back to light when storage fails. Do not add a marketing hero, 3D/neon treatment, heavy glass, or distracting looping motion.
- Project-local Impeccable is pinned under `.agents/skills/impeccable/**` for Codex and `.agent/skills/impeccable/**` for Antigravity. Automatic Impeccable hooks remain off because the authenticated Nyan hook is authoritative. The runner must verify exact provider metadata plus every tracked payload blob against `.impeccable/lock.json` in the exact UI worktree, and the UI prompt invokes `/impeccable polish`; the skill never widens the file grant.
- An Antigravity UI writer may change only the exact committed `admin/src/features/<task>/**` paths in its trusted task spec. It may choose presentation components, visual hierarchy, layout, component-local CSS, responsive behavior, light motion, and component-local interactions using coordinator-provided props, callbacks, and hooks.
...
- `READY_FOR_USER_TESTING` means the mock/disabled/false trial stack and its documented flows are ready for Nyan to evaluate. It does not authorize staging, production, credentials, purchases, top-ups, refunds, or new spend.
- `APPROVED_FOR_LIVE` is a separate owner decision after supplier conformance, deployment, backup/restore, rollback, secret, and money-safety evidence. Only Nyan can grant it.
